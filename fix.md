...
## 缺陷七：agent 没有自己的记忆，"自持"不是自持（严重，影响已发布数字）

**现象。** 检查全部 648 个第 2、3 轮 turn 的 prompt：

```
prompt 里含自己上一轮原文:   0 / 648  (0.0%)
prompt 里含被投递同伴原文: 576 / 648  ← 对照，证明检测方法有效
```

**没有任何 agent 能看到自己上一轮说过什么。** prompt 只有：命题 + 完整证据卷宗 +
投递给它的同伴上一轮输出。三个拓扑都如此。

更极端的是 chain 里的 A：它没有任何同伴输入，所以它的三轮 prompt **逐字相同**
（36/36 场）。A 的"三轮"是同一个查询采样三次，不是三轮辩论。

**后果——下面这些指标的名字和它们实际测的东西不一致：**

| 指标 | 名义含义 | 实际测的 |
|---|---|---|
| `自持 held` | agent 重提自己说过的话 | **重采样稳定性**：同一份卷宗再问一次，它是否给出同一条事实 |
| `采纳→留存` | 接住之后守住了 | 第 3 轮该事实**又一次**被投递过来，或者被重新推导出来 |
| `r1→r3 存活` | 事实活到了终局 | 事实在第 3 轮被**重新产生** |
| `峰值保留` | 系统自己淘汰自己 | 同上 |

**这不是说这些数字错了**，它们仍然是可复现的量；但"持续""保留""存活"这类词
预设了一个不存在的记忆机制。已发布的报告里凡是用这些词解释机制的地方都要改。

**特别地**：`2026-09-02-fact-flow-report.html` 第七节说"stance 降低了入口采纳但没有
...
## 缺陷八：chain 的两跳需要两轮，三轮只够传一次

**现象。** 三个拓扑用同一套时序规则：**每轮三人并行生成，只看上一轮投递给自己的内容**。

```
chain   A|1 ← 无     B|1 ← 无     C|1 ← 无
        A|2 ← 无     B|2 ← A|1    C|2 ← B|1
        A|3 ← 无     B|3 ← A|2    C|3 ← B|2
```

所以 A 的内容到 C 需要两轮（A|1 → B|2 → C|3）。三轮的设计里，**A→C 的两跳只有
一次机会**，而且 A 在第 2、3 轮产生的任何东西都永远到不了 C。

这不是 bug，是设计选择——保持三个拓扑的轮次语义一致，才能比较。但它让 "chain"
比这个词听起来弱得多，也是 chain 接触率只有 42.6% 的直接原因。

**这也解释了为什么 `2026-09-01` 那份分析里两跳样本只有 7 个事实**：结构上就只有
一次机会。

**替代设计（如果要更强的链）。** 轮内串行：A 先说，B 看到 A 的当轮输出再说，
C 看到 B 的当轮输出再说。那样每轮都能走完两跳，接触率变成 6/9，
而且**轮内连线就是合法的**——现在的可视化规则（轮内永不连线）是针对当前实现的，
换成串行设计就必须改。

**注意这会改变什么**：串行设计下 A、B、C 不再对称，位置效应无法用 neutral 面板
做干净对照（现在可以，因为轮内并行）。两种设计各有取舍，需要明确选一个并说明。

## 修的顺序

0. **先决定要不要给 agent 自己的历史**（缺陷七）。这决定了要不要重新生成辩论，
   也决定了下面这些抽取修复要不要等它一起做。
...
experiments/diagnostics/selfhist.py    缺陷七：prompt 里有没有自己的历史
experiments/diagnostics/delivery.py    缺陷八：三个拓扑的投递时序
```
