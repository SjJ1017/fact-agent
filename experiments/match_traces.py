...
def match_dir(a) -> int:
    ...
    # Comma-separated values are a union, so a corpus can be split across cards
    # by workload rather than by name -- the games here differ by more than
    # twofold in candidate count, so splitting evenly by count leaves one card
    # idle for half an hour.
    wanted = [w.strip() for w in a.only.split(",") if w.strip()]
    if wanted:
        seen: dict[str, Path] = {}
        for d in a.indir:
            for w in wanted:
                for f in d.glob(f"*{w}*{a.suffix}"):
                    seen[str(f)] = f
...
def main() -> int:
    ...
    ap.add_argument("--only", default="",
                    help="只匹配文件名里含这些子串的 trace，逗号分隔取并集，"
                         "例如 --only star 或 --only -01,-02,-04；"
                         "用于把一个语料按工作量分到两块卡上跑")
