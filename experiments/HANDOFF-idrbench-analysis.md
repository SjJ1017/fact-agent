...
## 一个可选方向：图结构（不是唯一方向，也不是必做）

下面这些是**已经想到的**线索，写在这里是为了你不必从零开始，**不是让你只做这些**。如果你在数据里看到别的更值得追的东西，那更好——这个项目最有价值的几次进展都来自没预料到的观察。看完数据再决定做什么，别被这一节框住。

之所以列图结构，是因为它是本项目相对 DelibTrace 一个结构性的优势：原子匹配做完之后才有图，而 DelibTrace 只能做线性的存活率变化。
...
再说一遍：以上是线索不是任务清单。你完全可以从别的角度切——事实的内容维度（哪类事实容易被传、哪类容易被丢）、时间维度（轮次效应、早期发言的优势）、语言维度（措辞变化本身的规律）、或者你在读原文时注意到的任何反常。**判断标准是「这个发现 DelibTrace 的设计能不能产出」，不是「它属不属于图分析」。**
